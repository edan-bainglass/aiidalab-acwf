...
from aiidalab_acwf.utils import get_pseudo_info
...
class WorkflowSummaryModel(ResultsComponentModel):
    ...
    def generate_report_text(self, report_dict):
        ...
        for calc in ("SCF", "NSCF", "Bands"):
            if f"K-point mesh distance ({calc})" in report_dict:
                kpoints_distances.append(str(report_dict[f"K-point mesh distance ({calc})"][0]))
    ...
    def generate_failure_report(self):
        """Generate a html for reporting the failure of the `AcwfAppWorkChain`."""
