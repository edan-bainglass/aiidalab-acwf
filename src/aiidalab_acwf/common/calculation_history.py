...
class CalculationHistory:
    def __init__(self, process_label: str):
    ...
    def load_data(self):
        """Fetch the AcwfAppWorkChain results using the QueryBuilder and
        ...
        for row in results:
            ...
            creation_time_str = creation_time.strftime("%Y-%m-%d %H:%M:%S") if creation_time else ""
            if creation_time:
            ...
            pk_with_link = f'<a href="./acwf.ipynb?pk={pk}" target="_blank">{pk}</a>'
            uuid_with_link = f'<a href="./acwf.ipynb?pk={pk}" target="_blank">{uuid[:8]}</a>'
            delete_link = f'<a href="./delete.ipynb?pk={pk}" target="_blank">Delete</a>'
            download_link = f'<a href="./download.ipynb?pk={pk}" target="_blank">Download</a>'
    ...
    def update_table_value(self, data_list):
        ...
        COLUMNS["creation_time_relative"]["hide"] = self.toggle_time_format.value != "Relative"
        COLUMNS["creation_time_absolute"]["hide"] = self.toggle_time_format.value != "Absolute"
    ...
    def apply_filters(self, _):
        ...
        selected_properties = [cb.description for cb in self.properties_box.children if cb.value]
